...
def combine_csv_files(filenames, outfilename, ignore=False, combine=False):
    with ExitStack() as stack:
        ...
        combined_fields = []
        for file in files:
            fields[file] = [field for field in csv.DictReader(file).fieldnames]
            combined_fields += [
                field for field in fields[file] if field not in combined_fields
            ]
        if ignore:
            readers = [csv.DictReader(file, fields[file]) for file in files]
        else:
            readers = [
                csv.DictReader(file, fields[file], restkey="extra", restval="")
                for file in files
            ]
        with open(outfilename, "w") as out_file:
            ...
            for reader in readers:
                ...
                for row in reader:
                    write_row = (
                        [row.get(field, "") for field in combined_fields]
                        + padding
                        + row.get("extra", [])
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--combine",
        help="Do not separate extra fields from different files. "
        "Extra fields will begin in the column after named fields.",
        action="store_true",
    )
    parser.add_argument("--ignore", help="Ignore extra fields.", action="store_true")
    args = vars(parser.parse_args())
    combine_csv_files(
        args["in_files"], args["out_file"], args["ignore"], args["combine"]
    )
